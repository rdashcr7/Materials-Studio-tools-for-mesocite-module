...
Structure_data = xlsread('Free KP PDB data.xlsx'); % storing strcuture data
...
for  i = 1 : No_structures
     phi(i) = acosd((Structure(i).x(2)*Structure(1).x(2)+Structure(i).y(2)*Structure(1).y(2))/sqrt(((Structure(i).x(2))^2+(Structure(i).y(2))^2)*((Structure(1).x(2))^2+(Structure(1).y(2))^2))); 
    theta(i) = acosd((Structure(i).x(2)*Structure(1).x(2)+Structure(i).z(2)*Structure(1).z(2))/sqrt(((Structure(i).x(2))^2+(Structure(i).z(2))^2)*((Structure(1).x(2))^2+(Structure(1).z(2))^2))); 
    psi(i) = acosd((Structure(i).z(2)*Structure(1).z(2)+Structure(i).y(2)*Structure(1).y(2))/sqrt(((Structure(i).z(2))^2+(Structure(i).y(2))^2)*((Structure(1).z(2))^2+(Structure(1).y(2))^2))); 

    % Rotation terms
    Term_1 = cosd(phi(i))*cosd(theta(i));
    Term_2 = cosd(phi(i))*sind(theta(i))*sind(psi(i)) - sind(phi(i))*cosd(psi(i));
    Term_3 = cosd(phi(i))*sind(theta(i))*cosd(psi(i)) + sind(phi(i))*sind(psi(i));
    Term_4 = sind(phi(i))*cosd(theta(i));
    Term_5 = sind(phi(i))*sind(theta(i))*sind(psi(i)) + cosd(phi(i))*cosd(psi(i));
    Term_6 = sind(phi(i))*sind(theta(i))*cosd(psi(i)) - cosd(phi(i))*sind(psi(i));
    Term_7 = -sind(theta(i));
    Term_8 = cosd(theta(i))*sind(psi(i));
    Term_9 = cosd(theta(i))*cosd(psi(i));

    % Temporary variables for rotation
    temp_x = Structure(i).x * Term_1 + Structure(i).y * Term_2 + Structure(i).z * Term_3;
    temp_y = Structure(i).x * Term_4 + Structure(i).y * Term_5 + Structure(i).z * Term_6;
    temp_z = Structure(i).x * Term_7 + Structure(i).y * Term_8 + Structure(i).z * Term_9;

    % Update with rotated coordinates
    Structure(i).x = temp_x;
    Structure(i).y = temp_y;
    Structure(i).z = temp_z;
end
...
ax.XAxis.Exponent = 0;
xtickformat('%.2f')
xlabel('Root Mean Square Deviation ()','FontSize', 30,'FontWeight','bold')
ylabel('Radius of Gyration ()','FontSize', 30, 'FontWeight', 'bold')
title('3-1 mapping No electrostatics','FontSize', 30, 'FontWeight', 'bold')
colorbar
...
for i = 1 : No_structures
    if abs(data_matrix(i,1)-mean(Rg))<=0.2 && abs(data_matrix(i,2)-mean(RMSD))<=2
        i
    end
    i = i + 1;
end
